-- ============================================================
-- KCEM EUM v1.7.2
-- shared_purchase_requests RPC bridge
--
-- 새 테이블 생성 없음 / 기존 테이블 ALTER 없음 / RLS 변경 없음
-- KCEM 공용 PIN 세션을 검증한 SECURITY DEFINER RPC만 추가
-- ============================================================

begin;

create or replace function public.kcem_shared_purchase_find_column(
    p_candidates text[]
)
returns text
language sql
...
as $$
    ...
    from unnest(p_candidates) with ordinality as x(name, ord)
...
create or replace function public.kcem_shared_purchase_column_type(
    p_column text
)
returns text
language sql
...
revoke all on function public.kcem_shared_purchase_find_column(text[]) from public, anon, authenticated;
revoke all on function public.kcem_shared_purchase_column_type(text) from public, anon, authenticated;
...
begin
    ...
    v_id := public.kcem_shared_purchase_find_column(
        array['id','request_id','purchase_request_id','purchase_id','shared_purchase_id','uuid']
    );
    ...
    if v_id is null then
        raise exception 'shared_purchase_requests ID 컬럼을 찾을 수 없습니다.';
    end if;
...
begin
    ...
    v_source := public.kcem_shared_purchase_find_column(array['source_site','site','source','origin_site','request_site']);
    v_status := public.kcem_shared_purchase_find_column(array['status','request_status','purchase_status']);
    v_priority := public.kcem_shared_purchase_find_column(array['priority','priority_level','urgency','priority_no']);
    v_date := public.kcem_shared_purchase_find_column(
        array['request_date','requested_date','request_day','requested_at','request_at','date_requested','request_dt','date']
    );
    v_content := public.kcem_shared_purchase_find_column(
    ...
    if v_id is null or v_source is null or v_status is null or v_priority is null
...
begin
    ...
    if v_id is null or v_source is null then
        raise exception 'shared_purchase_requests ID/source 컬럼을 확인하세요.';
    end if;
...
begin
    ...
    if v_id is null or v_status is null then
        raise exception 'shared_purchase_requests ID/status 컬럼을 확인하세요.';
    end if;
...
revoke all on function public.kcem_shared_purchase_list(text) from public;
revoke all on function public.kcem_shared_purchase_create(text,date,integer,text,text,numeric,text) from public;
revoke all on function public.kcem_shared_purchase_update(text,text,date,integer,text,text,numeric,text) from public;
revoke all on function public.kcem_shared_purchase_delete(text,text) from public;
revoke all on function public.kcem_shared_purchase_set_status(text,text,text) from public;
